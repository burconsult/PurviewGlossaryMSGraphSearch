# Purview Synchronization System

This document describes the components and workflow of the Purview Synchronization System.

## Overview

The system consists of two main components:
1.  **Connector App**: Connects to an external system (e.g., ServiceNow) to detect changes.
2.  **Azure Function App (`PurviewSyncFunc`)**: Listens for change events and updates Azure Purview accordingly.
...
*   **Indexes Items in Graph:**
    ...
    *   Pushes these `ExternalItem` objects to the selected Graph connection, either performing a full sync (all terms) or an incremental sync (only terms updated since the last run, based on `lastSync.txt`).
...
4.  **Create Graph Connection & Schema:**
    ...
    *   Choose option `1` to create a new connection. Provide a unique ID (e.g., `purviewglossary`), name, and description.
...
*   **License:** Consider adding a `LICENSE` file (e.g., MIT, Apache 2.0) to the root of the repository to define how others can use or contribute to this project. 